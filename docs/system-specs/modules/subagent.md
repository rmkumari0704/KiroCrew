...
- **The settle's PROPAGATION is owed by the reporter, not by the row's write**,
  so it runs on every arm of `taskq_settle` — the inline one, the posted one, and
  each one's `TaskStoreUnavailable` and unexpected-exception arms. The one-shot
  token means a propagation this settle skips is one nothing anywhere retries: a
  waiting parent is left parked on a wait no wake will ever end, and a cancelled
  parent's children keep running unnoticed. Both halves decide from the child ID
  they are handed rather than from the child's row (`on_child_terminal` adds it to
  the terminal set itself), so a refused terminal write does not change what they
  decide, and both absorb a store outage of their own. Pinned on both settle paths
