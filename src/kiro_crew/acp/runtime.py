...
class AcpRuntime:
    ...
    async def _spawn_admitted(self) -> None:
        ...
        try:
            ...
            raise AcpRuntimeError(str(exc)) from exc
        # Resolved BEFORE the process exists, right after the freshness gate above
        # took its snapshot, so the spec this reads is the generation the child
        # will load. A DerivedSpecStale raised here re-derives once, like every
        # other read on this path.
    ...
    async def _handshake_client_capabilities(self) -> dict[str, Any]:
        ...
        if self._tool_search is None or not self._harness.client_meta_settings:
            return base
